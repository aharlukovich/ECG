...
# Wybór pliku w panelu bocznym
st.sidebar.markdown("### 📁 Wybór pliku EKG")
selected_file = st.sidebar.selectbox(
    "Wybierz plik do analizy:",
    options=txt_files,
    index=0   # domyślnie pierwszy plik (spoczynkowy)
)

file_path = os.path.join(DATA_FOLDER, selected_file)

st.sidebar.info(f"Wybrano: **{selected_file}**")

#%%--------------------------------FUNKCJA WCZYTYWANIA-----------------------------

@st.cache_data
def load_my_data(file_path: str):
    if not os.path.isfile(file_path):
        st.error(f"Plik nie istnieje:\n{file_path}")
        st.stop()
...
with st.expander("🗃️ Pełna tabela segmentów QRS (uśredniony profil + wybrane segmenty)", expanded=False):
    ...
    cols_to_show = ['SREDNI_QRS'] + [col for col in df_qrs.columns if col.startswith('QRS_')][:10]
    
    display_df = df_qrs[cols_to_show].round(4)
    ...
    st.caption(f"Wyświetlono {len(cols_to_show)} kolumn z {df_qrs.shape[1]} dostępnych. "
               f"Cała tabela ma {df_qrs.shape[0]:,} wierszy × {df_qrs.shape[1]:,} kolumn.")
...
with col_main:
    if 'imf' in st.session_state:
        ...
        # Downsampling – chroni przed błędem "Invalid string length"
        ...
        fig_imfs = make_subplots(
            rows=n_imf + 1, 
            cols=1,
            subplot_titles=["Sygnał oryginalny (po filtracji Savgol)"] + [f"IMF {i+1}" for i in range(n_imf)],
            vertical_spacing=0.04
        )
        
        fig_imfs.add_trace(go.Scatter(x=czas_ds, y=ecg_raw_ds, mode='lines',
                                      line=dict(color=lekki_czerwony, width=2)), row=1, col=1)
        ...
        fig_clean.add_trace(go.Scatter(x=czas_ds, y=ecg_raw_ds, mode='lines',
                                       name='Po filtracji Savgol', line=dict(color='rgba(255,255,255,0.6)', width=2)))
        fig_clean.add_trace(go.Scatter(x=czas_ds, y=clean_ecg[::step], mode='lines',
                                       name='ECG bez modulacji', line=dict(color=zielony, width=3)))
        
        fig_clean.update_layout(
            height=380,
            title="Porównanie przed i po usunięciu modulacji oddechowej",
            xaxis_title="Czas [s]",
            yaxis_title="Amplituda [mV]",
            template="plotly_dark",
        ...
        with col_m1: st.metric("Średnia amplituda modulacji", f"{np.mean(np.abs(modulacja)):.4f} mV")
        with col_m2: st.metric("Maks. amplituda modulacji", f"{np.max(np.abs(modulacja)):.4f} mV")
        with col_m3: st.metric("Liczba IMF usuniętych", str(imfs_to_remove))
        with col_m4: st.metric("Liczba IMF łącznie", str(imf.shape[1]))
        
        # ==================== 3. Hilbert-Huang Transform (zoptymalizowany) ====================
        st.markdown("###### Hilbert-Huang Spectrum (HHT)")
        
        with st.spinner("Obliczanie Hilbert-Huang Transform (z downsamplingiem)..."):
            # Downsampling tylko do HHT – mocno redukuje rozmiar
            step_hht = max(1, len(czas) // 4000)   # ~4000 punktów w czasie → powinno zejść poniżej 100-150 MB
            imf_hht = imf[::step_hht, :]
            ...
            fig_hht.update_layout(
                title="Hilbert-Huang Spectrum – Rozkład częstotliwości w czasie",
                xaxis_title="Czas [s]",
        ...
        # ==================== 4. Eksport ====================
        st.markdown("---")
        txt_content = "czas[s]\tECG_clean\n" + "\n".join([f"{t:.3f}\t{c:.6f}" for t, c in zip(czas, clean_ecg)])
        
        st.download_button(
            label="📥 Pobierz oczyszczony sygnał EKG (.txt)",
            data=txt_content,
            file_name=f"clean_ecg_{selected_file}",
            mime="text/plain",
            use_container_width=True
        )
        
    else:
        st.info("👈 Naciśnij przycisk **Wykonaj dekompozycję EMD** po lewej stronie.")

st.markdown(f"""
    <hr style="margin-top: 20px; height:5px; border:none; background-color:{lekki_szary};"/>
""", unsafe_allow_html=True)
